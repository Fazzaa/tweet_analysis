...
anger_tweets = open(ANGER_PATH)
anticipation_tweets = open(ANTICIPATION_PATH)
disgust_tweets = open(DISGUST_PATH)
fear_tweets = open(FEAR_PATH)
joy_tweets = open(JOY_PATH)
sadness_tweets = open(SADNESS_PATH)
surprise_tweets = open(SURPRISE_PATH)
trust_tweets = open(TRUST_PATH)

punctuation = [ ",", "?", "!", ".", ";", ":", "\\", "/", "(", ")", "&", "_", "+", "=", "<", ">" ]

pos_emoticons = [ 'B-)',':)', ':-)', ":')", ":'-)", ':D', ':-D',':\'-)',":')",':o)',':]',':3',':c)',':>','=]','8)','=)',':}',':^)','8-D','8D','x-D','xD','X-D','XD','=-D','=D','=-3','=3','B^D',':-))',':*',':^*','( \'}{\' )','^^','(^_^)','^-^',"^.^","^3\^","\^L\^"]
